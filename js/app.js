 ...
 * Version Final (Stabilisiert & Segregation)
 */
...
// LOKALE BILDER
const HAZARD_LABELS = {
    "2.1": "img/class_2.1.png", "2.2": "img/class_2.2.png", "2.3": "img/class_2.3.png",
    "3":   "img/class_3.png", "4.1": "img/class_4.1.png", "4.2": "img/class_4.2.png",
    "4.3": "img/class_4.3.png", "5.1": "img/class_5.1.png", "5.2": "img/class_5.2.png",
    "6.1": "img/class_6.1.png", "6.2": "img/class_6.2.png", "8": "img/class_8.png", "9": "img/class_9.png"
};
...
/* =========================================
   HILFSFUNKTIONEN (MÜSSEN GANZ OBEN SEIN)
   ========================================= */

function checkLimit(maxLimit, currentQty) { 
    return (maxLimit !== 0 && maxLimit !== null) ? currentQty <= maxLimit : false; 
}
...
function getQItems() {
    ...
    rows.forEach(row => {
        ...
        let pgInput = row.querySelector('.q-pg').value;
        const pg = (pgInput === "null") ? null : pgInput;
        
        // Füge nur Items zur Liste hinzu, wenn UN-Nummer vorhanden ist
        if (un) { 
...
function checkSegregation(items) {
    if (items.length < 2) { return { is_compatible: true, message: "" }; }

    const classes = items.map(item => item.entry.class);
    
    const class3_present = classes.includes('3');
    const class8_present = classes.includes('8');
    if (class3_present && class8_present) {
        return { 
            is_compatible: false, 
            message: "⚠️ **INCOMPATIBLE CLASSES:** Stoffe der Klasse 3 (Entzündbare Flüssigkeiten) und der Klasse 8 (Ätzende Stoffe) dürfen **NICHT** zusammen in einer Verpackung transportiert werden. Bitte IATA 5.0.2.11 prüfen."
        };
    }

    const class5_1_present = classes.includes('5.1');
    const flammable_present = classes.includes('3') || classes.includes('4.1') || classes.includes('4.2') || classes.includes('4.3');
    if (class5_1_present && flammable_present) {
        return {
            is_compatible: false, 
            message: "⚠️ **INCOMPATIBLE CLASSES:** Oxidierende Stoffe (Klasse 5.1) sind mit entzündbaren Stoffen (Klasse 3/4) inkompatibel und dürfen nicht gemischt verpackt werden. Bitte Positionen trennen."
        };
    }
    return { is_compatible: true, message: "" };
}
...
function setupEventListeners() {
    ...
    document.querySelector('.close-modal').addEventListener('click', closePopup);
    window.onclick = function(event) { if (event.target == document.getElementById('pi-modal')) closePopup(); }
}
...
function handleSingleCheck(event) {
    ...
    if (!unNumber || packingGroupInput === "" || isNaN(netQtyInput)) {
        resultContainer.innerHTML = '<p style="color:var(--danger);">❌ Bitte alle Felder ausfüllen.</p>';
        return;
    }
    ...
    if (entry.lq_allowed) {
        if (entry.lq_max_qty && qtyInKg <= entry.lq_max_qty) { lqText = `✅ Möglich (Max. ${entry.lq_max_qty} kg/L)`; lqColor = "var(--success)"; }
        else { lqText = `⚠️ Menge zu hoch (Limit: ${entry.lq_max_qty} kg/L)`; lqColor = "var(--warning)"; }
    ...
    const paxPiLink = createPiLink(entry.pax_instruction); const caoPiLink = createPiLink(entry.cao_instruction);
    ...
    <table style="width:100%; margin-top:10px;"><tr style="background:var(--bg-body);"><th>Modus</th><th>Erlaubt?</th><th>Max. Menge / Pkg</th><th>Vorschrift (PI)</th></tr>
...
function calculateQValue(event) {
    event.preventDefault();

    const qItems = getQItems();
    const resultBox = document.getElementById('q-result');
    const warningDiv = document.getElementById('segregation-warning');

    if (qItems.length === 0) { resultBox.classList.add('hidden'); warningDiv.classList.add('hidden'); return; }

    const segregationResult = checkSegregation(qItems);
    
    if (!segregationResult.is_compatible) {
        resultBox.classList.add('hidden'); 
        warningDiv.innerHTML = segregationResult.message;
    ...
    for (let item of qItems) {
        ...
        const row = document.querySelector(`.q-un[value="${item.un}"]`).closest('.q-row'); 
        const qty = parseFloat(row.querySelector('.q-qty').value);
        const unit = row.querySelector('.q-unit').value;

        let maxQty = (mode === 'PAX') ? entry.pax_max_qty : entry.cao_max_qty;

        if (!maxQty || maxQty === 0) { 
            alert(`Fehler: ${entry.un_number} (${item.pg}) ist im Modus ${mode} verboten!`); 
            error = true; break; 
        }

        let currentQtyKg = (unit === 'g' || unit === 'ml') ? qty / 1000 : qty;
        let q_part = currentQtyKg / maxQty; Q += q_part;
        detailsHtml += `<li style="margin-bottom: 5px; border-bottom: 1px solid var(--border-color); padding-bottom:5px;"><strong>UN ${item.un}:</strong> Rechnung: ${currentQtyKg} / ${maxQty} = <strong>${q_part.toFixed(4)}</strong></li>`;
    }
    if (error) return;
